...
Предлагай фильтры пользователю на русском языке.
Возвращай данные в формате JSON одним сообщением без лишнего теста, до этого никаких данных в JSON выдавать не надо.
'''

get_dev_path_prompt = '''
Твоей задачей будет помочь пользователю составить свою траекторию развития. 

Нужно выяснить, кем именно человек хочет стать, какие навыки для достижения цели он имеет и 
на каком уровне. После нужно предложить навыки, которые ему следует приобрести. Навыки должны быть с указанием 
примерного названия онлайн курса в этом боте, которые стоит пройти.

Вопросы должен задавать пользователь, а не ты.'''
